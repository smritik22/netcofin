...
class PropertyController extends Controller
{
    ...
    public function __construct()
    {
        $this->property_types = [
            ...
            "agent_properties" => [
                "is_secondary" => true,
                "label" => "agent_properties",
                "label_secondary" => "agent"
            ],
            "buy" => [
                "is_secondary" => false,
    ...
    public function property_list(Request $request, $list_type = "")
    {
        ...
        if($list_type) {
            ...
            if ($breadcumb_data['is_secondary']) {
                ...
                } else if ($list_type == 'agent_properties') {
                    $agent_id = $request->input('agent');
                    $agent_details = MainUsers::find(decrypt($agent_id));

                    $PageTitle = $labels['agent_properties'];
                }
            }
        ...
        return view('frontEnd.property.list', compact('language_id', 'PageTitle', 'PageDescription', 'PageKeywords', 'WebmasterSettings', 'labels', 'breadcumb_title_arr', 'filters', 'areasqft_list', 'currency', 'latitude', 'longitude', 'area_id', 'list_type', 'property_id', 'property_detail', 'property_type', 'property_for', 'search', 'is_search', 'agent_id', 'agent_details'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->input('agent_id')) {
            $properties_query->where('agent_id', '=', decrypt($request->input('agent_id')));
        }
    ...
    public function add_property_submit(Request $request) {
        ...
        $condition_type_id = $request->input('condition');
        $completion_status_id = $request->input('completion_status');
        ...
        if(@$subscription_plan && $is_renew != 1) {
	...
	public function sendEmailAddProperty($language_id, $agent_email, $full_name, $mobile_no, $country_code, $property, $template_id, $url = "", $logo = "")
	{
		...
		$from_email = $setting['from_email'];
		$data = array('email' => $email, 'full_name' => $full_name, 'agent_email' => $agent_email, 'phone' => $mobile_no, 'country_code' => $country_code, 'url' => $url, 'id' =>  $template_id, 'language_id' => $language_id, 'logo' => $logo, 'from_email' => $from_email, 'property' => $property);
		// try {
    ...
    public function fetch_my_ads(Request $request) {
        ...
        $pagination_data = $properties->toArray();
        $html = view('frontEnd.property.loadMyads', compact('properties', 'labels', 'language_id', 'user_id', 'pagination_data'))->render();
    ...
    public function area_map(Request $request, $list_type = "") {
        $language_id = Helper::currentLanguage()->id;
        $labels = Helper::LabelList($language_id);
        ...
        return view('frontEnd.property.area.map', compact('language_id', 'PageTitle', 'PageDescription', 'PageKeywords', 'WebmasterSettings', 'labels', 'breadcumb_title_arr', 'currency', 'latitude', 'longitude', 'area_id', 'list_type', 'property_id', 'property_detail', 'property_type', 'property_for', 'search', 'is_search'));
