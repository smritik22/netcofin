...
@section('title', __('backend.editstate'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.editstate') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('state') }}">{{ __('backend.state') }}</a> / 
                    <span>{{ __('backend.editstate') }}</span>
            ...
            <div class="box-body">
                {{Form::open(['route'=>['state.update',$state->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'stateForm' ])}}

                <div class="personal_informations">
                    <div class="form-group row">
                        <label class="col-sm-2 form-control-label">{!!  __('backend.state_name') !!}</label>
                        <div class="col-sm-10">
                            <input type="text" name="state_name" id="state_name" class="form-control" placeholder="state Name" value="{{old('state_name',urldecode($state->name))}}" maxlength="100">
                        ...
                        <div class="col-sm-10">
                            <select name="country" id="country" value="" class="form-control">
                                ...
                                @foreach ($country_list as $key => $value )
                                    <option value="{{$value->id}}"  {{ ($state->country_id == $value->id) ? "selected" : "" }}>{{$value->name}}</option>
                                @endforeach
                            </select>
                            <span class="help-block">
