...
@section('title', __('backend.topicNew') . " " . __('backend.city'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.topicNew') }} {{ __('backend.city') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('city') }}">{{ __('backend.city') }}</a> / 
                    <span>{{ __('backend.topicNew') }} {{ __('backend.city') }}</span>
                </small>
            </div>
            <div class="box-tool">
                <ul class="nav">
            ...
            <div class="box-body">
                {{Form::open(['route'=>['city.store'],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'cityForm' ])}}

                <div class="personal_informations">
                    <div class="form-group row">
                        <label class="col-sm-2 form-control-label">{!!  __('backend.city_name') !!}</label>
                        <div class="col-sm-10">
                            <input type="text" name="city_name" id="city_name" class="form-control" placeholder="City Name" value="{{old('city_name')}}" maxlength="100">
