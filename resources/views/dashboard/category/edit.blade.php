...
@section('title', __('backend.editCategory'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.editCategory') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('categories') }}">{{ __('backend.category') }}</a> / 
                    <span>{{ __('backend.editCategory') }}</span>

                </small>
            </div>
            <div class="box-body">
                {{Form::open(['route'=>['category.update',$category->id],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'userForm' ])}}
                {{ csrf_field() }}
                <div class="personal_informations">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            {!! Form::text('name', old('name',urldecode($category->name)), ['class' => 'form-control', 'id' => 'name', 'maxlength' => '100']) !!}
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select name="businessowner" id="businessowner" value="" class="form-control">
                                {{-- <option value=""></option> --}}
                                @foreach ($businessOwners as $key => $value )
                                    <option value="{{$value->id}}" {{old('businessowner', $category->business_owner_id) == $value->id? "selected" : "" }}>{{$value->full_name}}</option>
                                @endforeach
                            </select>
                            <span class="help-block">
                                @if(!empty(@$errors) && @$errors->has('businessowner'))
                                    <span  style="color: red;" class='validate'>{{ $errors->first('businessowner') }}</span>
                                @endif
                            </span>
                        </div>
                    </div>
                                    ...
                                    <div class="col-sm-12 images">
                                        <div id="user_photo" class="col-sm-4 box p-a-xs">
                                            <a target="_blank" href="{{ $image_url . $category->image }}"><img
                                                    src="{{ $image_url . $category->image }}" class="img-responsive" style="width:200px; height:200px">
                                            </a>
                                            <br>
                                            <div class="delete">
                    ...
                    <div class="form-group row m-t-md">
                        <div class="offset-sm-2 col-sm-10">
                            ...
                            <a href="{{ route('categories')}}" class="btn btn-default m-t">
