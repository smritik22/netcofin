...
@section('title', __('backend.view_category'))
@push("after-styles")
    ...
    <div class="padding edit-package website-label-show">
        <div class="box">
            <div class="box-header dker">
                <h3>{{ __('backend.view_category') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                   <a href="{{ route('generalusers') }}">{{ __('backend.general_user_mngmnt') }}</a> /
                   <span>{{ __('backend.view_category') }}</span>
                </small>
            </div>
            <div class="box-body">
                {{Form::open(['route'=>['categories'],'method'=>'GET', 'files' => true,'enctype' => 'multipart/form-data' ])}}
                ...
                <div class="personal_informations">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 form-control-label">
                           <label>{{urldecode($category->name)}}</label>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 form-control-label">
                           <label>{!! Carbon::parse($category->updated_at)->format(env('DATE_FORMAT','Y-m-d') . ' h:i A') !!}</label>
                        </div>
                    </div>
                ...
                <div class="form-group row m-t-md">
                    <div class="offset-sm-2">
                        <a href="{{ route('categories') }}" class="btn btn-default m-t" style="margin: 0 0 0 0px">
