...
@section('title', __('backend.addCategory'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.addCategory') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <span>{{ __('backend.addCategory') }}</span>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="name" id="name" class="form-control"
                                placeholder="{{ __('backend.topicName') }}" value="{{old('name')}}" >
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select name="businessowner" id="businessowner" value="" class="form-control">
                                <option value=>Select Businessowner</option>
                                @foreach ($businessOwners as $key => $value )
                                    <option value="{{$value->id}}" {{old('businessowner') ? "selected" : "" }}>{{$value->full_name}}</option>
                                @endforeach
                            </select>
                            <span class="help-block">
