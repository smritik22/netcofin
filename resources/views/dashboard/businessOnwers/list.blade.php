...
@section('title', __('backend.business_owner'))
@section('content')
    ...
    <div class="padding website-label">
        <div class="box">
            ...
            <div class="box-header dker">
                <h3>{{ __('backend.business_owner') }}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <span>{{ __('backend.business_owner') }}</span>
                </small>
            </div>
            <div class="box-tool">
                <ul class="nav">
                    <li class="nav-item inline">
                        <a class="btn btn-fw primary" href="{{ route('businessOwner.create') }}">
                            <i class="material-icons">&#xe7fe;</i>
                            &nbsp; {{ __('backend.addBusinessOwner') }}
                        </a>
                    </li>
            ...
            {{ Form::open(['route' => 'businessOwner.updateAll', 'method' => 'post', 'id' => 'updateAll']) }}
            ...
            <div class="table-responsive">
                <table class="table table-bordered m-a-0" id="businessOwners">
                    <thead class="dker">
                        <tr>
                            ...
                            <th>{{ __('backend.profile_image') }}</th>
...
@push('after-scripts')
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            function load_data() {
                var action_url = "{!! route('businessOwner.anyData') !!} ";
                var businessOwner_type = "{!! $businessOwner_type !!}";
                var dataTable = $('#businessOwners').DataTable({
                    'processing': true,
                    'serverSide': true,
                    'serverMethod': 'post',
                    columnDefs: [{
                    ...
                    ajax: {
                        'url': action_url,
                        'data': function(data, businessOwner_type) {
                            var businessOwner_type = $('#searchBybusinessOwnerType').val();
                            data.searchBybusinessOwnerType = businessOwner_type;
                            data.businessOwner_type = businessOwner_type;
                        }
                    },
                    columns: [{
                            data: 'checkbox',
                ...
                $("#searchBybusinessOwnerType").change(function() {
                    dataTable.draw();
                });
