...
@section('title', __('backend.addBusinessOwner'))
@push("after-styles")
        ...
        <div class="box">
            <div class="box-header dker">
                <h3><i class="material-icons">
                        &#xe02e;</i> {{ __('backend.addBusinessOwner') }}
                </h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a href="{{ route('businessOwners') }}">{{ __('backend.general_user_mngmnt') }}</a> / 
                    <span>{{ __('backend.addBusinessOwner') }}</span>

                </small>
            </div>
            <div class="box-body">
                {{Form::open(['route'=>['businessOwner.store'],'method'=>'POST', 'files' => true,'enctype' => 'multipart/form-data', 'id' => 'userForm' ])}}
                {{ csrf_field() }}
                <div class="personal_informations">
                    ...
                    <div class="form-group row">
                        ...
                        <input type="text" name="username" id="username" class="form-control"
                                placeholder="{{ __('backend.user_name') }}" maxlength="50" value="{{ old('username')}}">
                            <span class="help-block">
                        ...
                        <div class="col-sm-10">
                            <select name="businessowner_type" id="businessowner_type" value="" class="form-control">
                                ...
                                @foreach ($businessOwner_types as $key => $value )
                                    <option value="{{$value->id}}" {{old('businessowner_type', "selected")  }}>{{$value->type_name}}</option>
                                @endforeach
                            </select>
                            <span class="help-block">
